game.ImpEntity = me.Entity.extend({
    init:function (x, y, settings) {
        //Save the goblin settings from tiled so we can spawn more at a future point.
        if(me.game.impSettings === undefined)
            me.game.impSettings = settings;
        settings = me.game.impSettings;
        settings.image = "impchilled";
        ...
        //This generates a random animation speed between 140 and 160
        this.animationSpeed = 150 + Math.floor(Math.random()*11)*(Math.random>0.5?-1:1);
        this.renderable.addAnimation("walkdown", [0, 1, 2, 3], this.animationSpeed);
        this.renderable.addAnimation("walkright", [8, 9, 10, 11], this.animationSpeed);
        this.renderable.addAnimation("walkup", [4, 5, 6, 7], this.animationSpeed);
        this.renderable.addAnimation("walkleft", [12, 13, 14, 15], this.animationSpeed);
        this.renderable.addAnimation("death", [5, 9, 1, 13, 5, 9, 1, 13, {name: 5, delay: Infinity}], this.animationSpeed);
        this.renderable.addAnimation("chilldown", [16, 17, 18, 19], this.animationSpeed*2);
        this.renderable.addAnimation("chillright", [24, 25, 26, 27], this.animationSpeed*2);
        this.renderable.addAnimation("chillup", [20, 21, 22, 23], this.animationSpeed*2);
        this.renderable.addAnimation("chillleft", [28, 29, 30, 31], this.animationSpeed*2);

        //Wave Bonus Data
        this.wave = game.data.wave;         //Current wave
        this.hp_buff = 10;                  //Bonus HP per wave
        this.speed_buff = 3;                //Bonus Speed per wave
        this.armor_buff = .25;              //Bonus Armor per wave              

        //Tower Defense Variables
        this.base_hp = 150;                 //Base Health Pool
        this.max_hp = this.base_hp + this.wave*this.hp_buff;    //Total Health Pool
        this.health_points = this.max_hp;   //Health Remaining
        this.bounty = 25;                   //Gold Dropped
        this.base_armor = 3;                //Base Armor value
        this.armor = this.base_armor + this.wave*this.armor_buff;   //Damage Mitigation
        this.enrage = 0;                    //Velocity Gained on health loss
        ...
        this.power = game.data.impPower;    //Player "lives" lost upon enemy escape.
        this.forked = false;                //has entity been diverted yet, RE: level02


        //Entity base speed value
        this.vel_init = 15;                 //Initial value for velocity pre RNG and buffs
        this.vel_rand = 3;                  //Vel +- 2
        this.baseVelocity = this.vel_init + this.wave*this.speed_buff + Math.floor(Math.random()*this.vel_rand)*(Math.random>0.5?-1:1); //Base velocity calculation
        this.velocity = this.baseVelocity;

        //Set tick delay (Time until the unit starts moving)
        if(settings.tickDelay === undefined)
        {
            this.health_points = 0;     //Kill the enemy that was spawned from tiled...
    ...
    update : function (dt) {
        ...
        if(!this.alive){
            if(this.deathTimer === undefined){
                this.deathTimer = 0;
                if(this.power>0)
                    me.audio.play("imp_death",false,null,0.5);
                if(!this.inViewport)
        ...
        //Start death animation if necessary
