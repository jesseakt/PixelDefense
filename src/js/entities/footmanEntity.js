game.FootmanEntity = me.Entity.extend({
    init:function (x, y, settings) {
        //Save the footman settings from tiled so we can spawn more at a future point.
        if(me.game.footmanSettings === undefined)
            me.game.footmanSettings = settings;
        settings = me.game.footmanSettings;
        settings.image = "footmanchilled";
        ...
        this.power = game.data.footmanPower; //Player "lives" lost upon enemy escape.
        this.forked = false;                //has entity been diverted yet, RE: level02

        //Entity base speed value
        this.vel_init = 25;                 //Initial value for velocity pre buff and rng
        this.vel_rand = 4;                  //RNG +- 3
    ...
    update : function (dt) {
        ...
        if(!this.alive){
            if(this.deathTimer === undefined){
                this.deathTimer = 0;
                if(this.power>0)
                    me.audio.play("footman_death",false,null,0.5);
                if(!this.inViewport)
